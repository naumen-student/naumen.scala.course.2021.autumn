...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def getMinPrimeNum(number: Int): Int = {
            var result = number
            var flag = true
        ...
        var _number = number
        while(_number > 1){
            val prime = getMinPrimeNum(_number)
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, procFunc: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double = {
        procFunc(leftVec0, leftVec1) + procFunc(rightVec0, rightVec1)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toArray
          .map(ball => (ball._1, ball._2._1 * ball._2._2))
          .sortBy(ball => ball._2)
          .map(ball => ball._1)
          .toSeq
    }
