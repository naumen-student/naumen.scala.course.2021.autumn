...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        var multiplier = 2
        var seq:Seq[Int] = Seq()
        while (i > 1 && multiplier * multiplier <= number) {
            if (i % multiplier == 0) {
                if (!seq.contains(multiplier)) {
                    seq = seq :+ multiplier
                }
                i = i / multiplier
            }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D,
                  func: (Vector2D, Vector2D) => Double, rightVec0:
                  Vector2D, rightVec1: Vector2D) = {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        var weighedBalls: Map[String, Double] = Map()
        for (b <- ballsArray) {
            val volume = (4.0/3) * java.lang.Math.PI * b._2._1 * b._2._1 * b._2._1
            val weight = volume * b._2._2
            weighedBalls = weighedBalls + (b._1 -> weight)
        }
        val sortedWeighedBalls = ListMap(weighedBalls.toSeq.sortWith(_._2 < _._2): _*)
        names = sortedWeighedBalls.keys.toSeq
        names
    }
