...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def isPrime(number: Int): Boolean = !Range(2, sqrt(number).toInt + 1).exists(i => number % i == 0)
        Range(2, number + 1) .filter(i => number % i == 0 && isPrime(i))
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def getWeight(radius: Int, density: Double) = density * 4/3 * PI * radius * radius * radius
        ballsArray
          .map(ball => (ball._1, getWeight(ball._2._1, ball._2._2)))
          .toSeq
          .sortBy(x => x._2)
          .map(x => x._1)
    }
