object Exercises {
    ...
    def divBy3Or7(iFrom: Int, iTo: Int): Seq[Int] = (iFrom to iTo).filter(x => x % 3 == 0 || x % 7 == 0)
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        while (number_ > 1)
            {
                if (number_ % denominator == 0) {
                    result = Seq(denominator) ++ result
                    number_ /= denominator
                } else
                    denominator += 1
            }
        result.reverse.distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.toSeq.map((ball) => (ball._1, ball._2._1 * java.lang.Math.PI * ball._2._2 * 4 / 3)).sortBy(ball => ball._2).map(ball => ball._1)
