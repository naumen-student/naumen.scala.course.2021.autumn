...
object Exercises {
  ...
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  @tailrec
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int, accumulator: Int = 0): Long = {
    if (iFrom == iTo + 1)
      accumulator
    else if (iFrom % 3 == 0 || iFrom % 5 == 0)
      sumOfDivBy3Or5(iFrom + 1, iTo, accumulator + iFrom)
    else
      sumOfDivBy3Or5(iFrom + 1, iTo, accumulator)
  }
  ...
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def primeFactor(number: Int, currentFactor: Int = 2, accumulator: Seq[Int] = Seq()): Seq[Int] = {
    if (number / currentFactor == 1 && number % currentFactor == 0)
      1 +: accumulator :+ currentFactor
    else if (number / currentFactor == -1 && number % currentFactor == 0)
      -1 +: accumulator :+ currentFactor
    else if (isPrime(currentFactor) && number % currentFactor == 0)
      primeFactor(number / currentFactor, findNextPrime(currentFactor), accumulator :+ currentFactor)
    else
      primeFactor(number, findNextPrime(currentFactor + 1), accumulator)
  }
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, scalar: Int, rightVec0: Vector2D, rightVec1: Vector2D) = {
    //Функция sumScalars должна вычислять сумму скалярных произведений для пар векторов scalar(leftVec0, leftVec1) + scalar(rightVec0, rightVec1).
    if (scalar == this.scalar)
      this.scalar(leftVec0, leftVec1) + this.scalar(rightVec0, rightVec1)
    //Функция sumCosines должна вычислять сумму косинусов углов между парами векторов cosBetween(leftVec0, leftVec1) + cosBetween(rightVec0, rightVec1).
    else if (scalar == this.cosBetween)
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    val hm = ballsArray.map(ball => {
      (ball._1, getBallsWeight(ball._2._1, ball._2._2))
    })
    hm.toSeq.sortBy(_._2).map(elem => elem._1)
  }

  def getBallsWeight(radiusInCentimeters: Int, density: Double): Double = {
    //m = p * v
    // Не привожу единицы в СИ, т.к в контексте задачи это не нужно
    val p = density
    val v = (4 / 3) * Math.PI * radiusInCentimeters
    val m = p * v
    m
  }
