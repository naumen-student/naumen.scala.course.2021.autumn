object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, sbf: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) =
        sbf(leftVec0, leftVec1) + sbf(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.map(x => (x._1, x._2._1 * x._2._1 * x._2._1 * x._2._2 * java.lang.Math.PI * (4.0/3))).sortBy(x => x._2).map(x => x._1)
    }
