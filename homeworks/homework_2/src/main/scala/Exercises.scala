object Exercises{
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def primeFactor(number: Int, from: Int, to: Int): Seq[Int] = {
            Range(from, to, 2).find(n => number % n == 0) match {
                case n: Some[Int] => {
                    val divisors = primeFactor(number / n.value, n.value, to)
                    if (!divisors.contains(n.value))
                        return n.value +: divisors
                    divisors
                }
                case _ => {
                    if (number == 1)
                        ...
                        primeFactor(number, from + 1, to)
                }
            }
        }

        primeFactor(number, 2, Math.sqrt(number).toInt + 1)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
      ballsArray
        ...
        .sortBy(elem => Math.pow(elem._2._1, 3) * elem._2._2)
        .map(x => x._1)
    }
}