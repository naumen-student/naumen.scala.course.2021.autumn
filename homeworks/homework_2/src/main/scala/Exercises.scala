object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def iter(num: Int, divider: Int, acc: Seq[Int]): Seq[Int] = {
            if (num == 1) acc.distinct
            else if (isPrime(divider)) {
                if (num % divider == 0)
                    iter(num / divider, 2, acc :+ divider)
                else
                    iter(num, divider + 1, acc)
            }
            else iter(num, divider + 1, acc)
        }
