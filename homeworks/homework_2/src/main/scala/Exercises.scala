...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        val possibleDividers = (num: Int) => for {i <- 2 to num-1
                                                  if num % i == 0
             } yield i
        possibleDividers(number).filter(n => { possibleDividers(n).forall(d => n % d != 0)})
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val mass = (r: Int, d: Double) => d * 4d / 3d * PI * r * r * r
        ballsArray.toSeq.sortBy(v => mass(v._2._1, v._2._2)).map(v => v._1)
    }
