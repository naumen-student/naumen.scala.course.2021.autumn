...
object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, typeOfFunc: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double =
        typeOfFunc(leftVec0, leftVec1) + typeOfFunc(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val weight = (for ((title, (radius, density)) <- ballsArray)
            yield { title -> 4 * java.lang.Math.PI * Math.pow(radius, 3) * density / 3}).toSeq.sortBy(_._2)

        for ((title, value) <- weight)
            yield title
    }
