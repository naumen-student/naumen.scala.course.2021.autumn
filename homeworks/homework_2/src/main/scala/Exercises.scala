object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def getPrimes(number: Int, from: Int): Set[Int] = {
            for {i <- from to number
                 if number % i == 0
                 } return Set(i) ++ getPrimes(number / i, i)
            Set()
        }

        getPrimes(number, 2).toSeq
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.keys.toSeq.sortBy(u => scala.math.pow(ballsArray(u)._1, 3) * ballsArray(u)._2)
    }
