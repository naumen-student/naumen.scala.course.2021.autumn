object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def factor(n: Int, i: Int): Seq[Int] = {
            (n % i == 0, n == 1) match {
                case (true, false) => factor(n / i, i) :+ i
                case (false, false) => factor(n, i + 1)
                case (false, true) => Seq(i)
            }
        }
        factor(number, 2).distinct.sorted
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] ={
        ballsArray.mapValues(parameters => (4 * java.lang.Math.PI * java.lang.Math.pow(parameters._1, 3)) / 3 * parameters._2)
          .toSeq.sortBy(_._2).map(x => x._1)
    }
