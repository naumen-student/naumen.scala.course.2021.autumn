...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        var prime = 2
        var div = number.abs
        while (div >= prime * prime) {
            if (div % prime != 0)
                prime += 1
            else {
                div /= prime
                if (prime > 1 && (result.isEmpty || result.last != prime)) result += prime
            }
        }
        if (div > 1 && (result.isEmpty || result.last != div)) result += div
        result
    }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, flatten: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double =
        flatten(leftVec0, leftVec1) + flatten(rightVec0, rightVec1)
