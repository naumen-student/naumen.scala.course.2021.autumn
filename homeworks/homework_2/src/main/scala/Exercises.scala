object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        var num = number
        var div = 2
        while (num > 1 && div * div  <= number) {
            if (num % div == 0) {
                if (!buffer.contains(div)) buffer = buffer :+ div
                num /= div
            }
            else {
                div += 1
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        val coef = 4 / 3
        ballsArray.map(ball => (ball._1, coef * pi * scala.math.pow(ball._2._1, 3) * ball._2._2)).toSeq.sortBy(
            x => x._2).map(x => x._1)
    }
}
