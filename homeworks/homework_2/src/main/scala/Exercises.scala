object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        primes.sortWith((x, y) => x < y).distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        def getVolume(radius: Int): Double = {
            4 * pi * radius * radius * radius / 3
        }

        def getWeight(radius: Int, density: Double): Double = {
            getVolume(radius) * density
        }

        balls.toSeq.sortWith((x, y) => getWeight(x._2._1, x._2._2) < getWeight(y._2._1, y._2._2)).map(x => x._1)
    }
