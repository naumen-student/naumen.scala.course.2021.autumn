object Exercises {
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).iterator.filter(i => i % 3 == 0 || i % 5 == 0).sum
    ...
    def primeFactor(number: Int): List[Int] = {
        (2 to number)
          .collectFirst { case i if number % i == 0 =>
              val tail = primeFactor(number / i)
              if (tail.headOption.contains(i)) tail else i :: tail
          }.getOrElse(Nil)
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.toSeq.map { case (n, (r, d)) => (n, (r^3) * d) }.sortBy(_._2).map(_._1)
