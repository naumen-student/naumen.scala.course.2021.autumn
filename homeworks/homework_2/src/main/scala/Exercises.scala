...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
       Stream.range(iFrom, iTo+1).filter(n =>  n % 3 == 0 || n % 5 == 0).sum
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        val start = InternalResult(number, Set.empty[Int])
        Stream
          .range(2, Math.floor(Math.sqrt(number)).toInt)
          .foldLeft(start)((acc, el) => divWhile(acc, el))
          .res.toSeq
    }

    @tailrec
    def divWhile(internalResult: InternalResult, el: Int): InternalResult = {
        if (internalResult.number % el==0)
            divWhile(InternalResult(internalResult.number/el, internalResult.res + el), el)
        else internalResult
    }

    case class InternalResult(number: Int, res: Set[Int])
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, f: (Vector2D, Vector2D) => Double,
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.toList.sortBy {
            case (_, (r, density)) =>
            val V = 4d/3*Math.PI*Math.pow(r, 3)
            V * density
        }.map(_._1)
