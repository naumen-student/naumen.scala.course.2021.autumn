...
object Test extends TestSuite{
    ...
    val tests = Tests{
        ...
        'test_sumScalars - {
            assert(Exercises.sumScalars(Vector2D(0, 0), Vector2D(1, 0), Vector2D(0, 0), Vector2D(2, 2)) == 0.0)
            assert(Exercises.sumScalars(Vector2D(0, 0), Vector2D(1, 1), Vector2D(2, 3), Vector2D(4, 5)) == 23.0)
            assert(Exercises.sumScalars(Vector2D(1, 0), Vector2D(1, 0), Vector2D(2, 3), Vector2D(4, 5)) == 24.0)
        }
        'test_sumCosines - {
