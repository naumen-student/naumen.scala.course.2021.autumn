...
object Test extends TestSuite{
    ...
    val tests = Tests{
        ...
        'test_sumByFunc - {
            val a = Exercises.Vector2D(1, 10)
            val b = Exercises.Vector2D(-100, 12)
            val c = Exercises.Vector2D(1, 10)
            val d = Exercises.Vector2D(-100, 12)
            assert(Exercises.sumScalars(a, b, c , d) == Exercises.scalar(a, b) + Exercises.scalar(c, d))
            assert(Exercises.sumCosines(a, b, c , d) == Exercises.cosBetween(a, b) + Exercises.cosBetween(c, d))
        }

        'test_sortByHeavyweight - {
            println(Exercises.sortByHeavyweight(Exercises.balls))
            assert(Exercises.sortByHeavyweight(Exercises.balls) ==
              List("Tin", "Platinum", "Nickel", "Aluminum", "Titanium", "Lead", "Sodium", "Uranium",
