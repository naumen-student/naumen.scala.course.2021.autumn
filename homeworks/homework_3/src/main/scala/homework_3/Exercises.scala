...
object Exercises {
    ...
    def max2(xs: Seq[Int]): Seq[Int] = {
        if (xs.isEmpty) Seq(0) else Seq(xs.max)
    }
     ...
     */
    class CustomClass(x: Int, y: Int) extends Iterable[Int] {
        def iterator: Iterator[Int] = Iterator(x, y)
    }
    def sum1(x: Int, y: Int): Int = sumIntegers(List(x, y))
    def sum2(x: Int, y: Int): Int = sumIntegers(Seq(x, y))
    def sum3(x: Int, y: Int): Int = sumIntegers(new CustomClass(x, y))
}
