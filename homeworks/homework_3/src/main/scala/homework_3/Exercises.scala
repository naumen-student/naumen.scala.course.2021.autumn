...
object Exercises {
  ...
  def prettyBooleanFormatter1(x: Any): String = {
    if (x.isInstanceOf[Boolean]) {
      val out = if (x.asInstanceOf[Boolean]) "правда" else "ложь"
      out
    } else {
      x.toString
    }
  ...
  def prettyBooleanFormatter3(x: Any): String = {
    x match {
      case bool: Boolean =>
        val out = if (bool) "правда" else "ложь"
        out
      case _ =>
        x.toString
    }
   ...
   */
  class MyInts(x: Int, y: Int) extends Iterable[Int] {
  ...
  def sum3(x: Int, y: Int): Int = sumIntegers(new MyInts(x, y))
