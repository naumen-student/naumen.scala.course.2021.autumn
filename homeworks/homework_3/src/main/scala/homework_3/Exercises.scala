...
object Exercises {
  ...
  def prettyBooleanFormatter3(x: Any): String = {
    val className = x.getClass.getSimpleName.toLowerCase()
    val booleanClassName = true.getClass.getSimpleName
  ...
  def max2(xs: Seq[Int]): Seq[Int] = xs.reduceOption(_ max _).toSeq

  def max3(xs: Seq[Int]): Option[Int] = xs.reduceOption(_ max _)

  case class MyPair(first: Int, second: Int) extends Iterable[Int] {
    override def iterator: Iterator[Int] = Seq(first, second).toIterator
  }
