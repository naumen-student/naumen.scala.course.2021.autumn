...
package homework_3{
    ...
    object Exercises {
        ...
        def prettyBooleanFormatter1(x: Any): String =
            x match {
                ...
                case _ => return x.toString
            }

        def prettyBooleanFormatter2(x: Any): String = {
            if (x.isInstanceOf[Boolean]) {
                val castedX = x.asInstanceOf[Boolean]
                if (castedX) "правда"
                else "ложь"
            }
            else
        ...
        def prettyBooleanFormatter3(x: Any): String = {
            ...
            if(xClass.toString == "class java.lang.Boolean") {
                val castedX = x.asInstanceOf[Boolean]
                if(castedX) "правда" else "ложь"
            }
            else x.toString
        }
         ...
         */
        def max1(xs: Seq[Int]): Int = xs.headOption match {
            case None => -1
            case _ => xs.max
        }

        def max2(xs: Seq[Int]): Seq[Int] = xs.headOption match {
            case None => Seq()
            case _ => Seq(xs.foldLeft(xs.head)((r, e) => if(e > r) e else r))
        }
    ...
    case class MyClass(x:Int, y:Int) extends Iterable[Int]{
        def iterator: Iterator[Int] = new Iterator[Int] {
            var innerBuff = List(x,y)
            def hasNext: Boolean = innerBuff.headOption match {
                case None => false
                case _ => true
            }

            def next(): Int ={
                val a = innerBuff.head
                innerBuff = innerBuff.tail
                a
