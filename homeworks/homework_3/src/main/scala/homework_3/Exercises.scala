...
object Exercises {
     ...
     */
    def prettyBooleanFormatter1(x: Any): String = if (x.isInstanceOf[Boolean]) printBoolean(x.asInstanceOf[Boolean]) else x.toString
    ...
    def prettyBooleanFormatter3(x: Any): String = if (x.getClass.getTypeName == "java.lang.Boolean") printBoolean(x.asInstanceOf[Boolean]) else x.toString

    def printBoolean(x: Boolean): String = if (x) "правда" else "ложь"
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(MyIntegers(x, y))

    case class MyIntegers(x: Int, y: Int) extends Iterable[Int] {
