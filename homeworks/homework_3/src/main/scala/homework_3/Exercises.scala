...
object Exercises {
    ...
    def max1(xs: Seq[Int]): Int = {
        if (xs.length == 0) throw new IllegalArgumentException("Collection is empty") else xs.max
    }

    def max2(xs: Seq[Int]): Seq[Int] = {
        if (xs.length == 0) Seq() else Seq(xs.max)
    }
     ...
     */
    class Pair(val first: Int, val second: Int) extends Iterable[Int]{
