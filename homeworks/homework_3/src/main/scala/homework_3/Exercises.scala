...
object Exercises {
     ...
     */
    def max1(xs: Seq[Int]): Int = if (xs.nonEmpty) xs.max else 0

    def max2(xs: Seq[Int]): Seq[Int] = if (xs.nonEmpty) Seq(xs.max) else Seq[Int]()
     ...
     */
    def sum1(x: Int, y: Int): Int = sumIntegers(IntIter(x, y))
    def sum2(x: Int, y: Int): Int = sumIntegers(Seq(x, y))
    def sum3(x: Int, y: Int): Int = sumIntegers(List(x, y))

    case class IntIter(x: Int, y: Int) extends Iterable[Int]{
