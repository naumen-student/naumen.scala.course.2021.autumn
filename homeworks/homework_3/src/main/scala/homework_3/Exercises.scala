...
object Exercises {
    ...
    def prettyBooleanFormatter1(x: Any): String =
        {
            ...
            if (x.getClass.getName == "java.lang.Boolean")
                res = if(x == true) "правда" else "ложь"
            else
                res = x.toString
            res
        }
    ...
    def max1(xs: Seq[Int]): Int = {
        val res = if (xs.nonEmpty) Some(xs.max)  else None
        res.getOrElse(0)
    }
    ...
    def sum2(x: Int, y: Int): Int = sumIntegers(IndexedSeq(x,y))
    def sum3(x: Int, y: Int): Int = sumIntegers(MyOwnCollectionType(x,y))

    case class MyOwnCollectionType(x: Int, y: Int) extends Iterable[Int]
    {
