...
object Exercises {
    ...
    def prettyBooleanFormatter3(x: Any): String = {
        val boolValues = Map(true -> "правда", false -> "ложь")
        if (x.isInstanceOf[Boolean] && boolValues.contains(x.asInstanceOf[Boolean]))
            boolValues(x.asInstanceOf[Boolean])
        else
            x.toString
    }
    ...
    def max3(xs: Seq[Int]): Option[Int] = if (xs.isEmpty) Some(xs.max) else None
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(ComplexNum(x, y))

    case class ComplexNum(x: Int, y: Int) extends Iterable[Int] {
