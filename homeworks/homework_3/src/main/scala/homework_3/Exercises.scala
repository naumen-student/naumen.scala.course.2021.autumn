...
object Exercises {
    ...
    def prettyBooleanFormatter3(x: Any): String = {
        if (x.getClass.toString == "boolean")
            if (x.asInstanceOf[Boolean]) "правда" else "ложь"
        else
            x.toString
    }
    ...
    def max1(xs: Seq[Int]): Int = {
        if (xs.isEmpty) xs.hashCode() else xs.max
    }
     ...
     */
    class mySeq (x: Int, y: Int) extends Iterable[Int]{
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(new mySeq(x, y))
