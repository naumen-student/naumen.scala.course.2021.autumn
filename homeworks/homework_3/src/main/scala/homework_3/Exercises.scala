...
object Exercises {
  ...
  def prettyBooleanFormatter1(x: Any): String =
    if (x.isInstanceOf[Boolean]) translate(x.asInstanceOf[Boolean]) else x.toString

  def prettyBooleanFormatter2(x: Any): String =
    if (x.getClass.getSimpleName == "Boolean") translate(x.asInstanceOf[Boolean]) else x.toString
  ...
  def translate(x: Boolean): String = if (x) "правда" else "ложь"
