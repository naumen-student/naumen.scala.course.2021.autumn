...
object Exercises {
  ...
  def prettyBooleanFormatter1(x: Any): String = {
    x match {
      case true => translateBoolInRussian(x.asInstanceOf[Boolean])
      case false => translateBoolInRussian(x.asInstanceOf[Boolean])
      case _ => x.toString
    }
  }

  def prettyBooleanFormatter2(x: Any): String =
    if (x.getClass.getSimpleName == "Boolean") translateBoolInRussian(x.asInstanceOf[Boolean]) else x.toString

  def prettyBooleanFormatter3(x: Any): String =
    if (x.isInstanceOf[Boolean]) translateBoolInRussian(x.asInstanceOf[Boolean]) else x.toString

  def translateBoolInRussian(x: Boolean): String = if (x) "правда" else "ложь"
  ...
  def sum3(x: Int, y: Int): Int = sumIntegers(CustomIterable(x, y))

  case class CustomIterable(x: Int, y: Int) extends Iterable[Int] {
