...
object ListOps {
  ...
  @tailrec
  implicit def reverse[T](buffer: DataList[T])(list: DataList[T]): DataList[T] = list match {
  ...
  def collect[A, B](f: PartialFunction[A, B]): DataList[A] => DataList[B] =
    list => map(f)(filter(elem => f.isDefinedAt(elem))(list))
}