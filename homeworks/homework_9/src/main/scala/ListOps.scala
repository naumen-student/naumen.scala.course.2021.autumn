...
object ListOps extends App {
  ...
  def sum[T : Numeric](list: DataList[T]): T = {
    ...
    foldOption(sumT)(list).getOrElse(0.asInstanceOf[T])
  }
  ...
  @tailrec
  final def reverse[T](buffer: DataList[T])(l: DataList[T]): DataList[T] = l match {
   ...
   */
  def collect[A, B](f: PartialFunction[A, B]): DataList[A] => DataList[B] = seq => map(f)(filter(elem => f.isDefinedAt(elem))(seq))
}